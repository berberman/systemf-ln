...
-- Example: `∀. 0 -> 0` is a locally closed type
...
-/
theorem lcAtTy_of_open {T : Ty} {X : Name} {k : ℕ}
...
-/
lemma lcAtTy_open_inv_aux {k : ℕ} {T U : Ty}
...
-/
lemma lcAtTy_open_inv {T : Ty} {X : Name} (h : LcAtTy 1 T) : LcAtTy 0 (T⟪$T X⟫) := by
  apply lcAtTy_open_inv_aux
  · assumption
  · constructor
...
/-
  Substitution preserves locally closeness of types
-/
