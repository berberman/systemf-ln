...
/-
  The free variables in a context `Γ` are the union of free variables in all types in `Γ`.
-/
...
inductive WfContext : Context → Prop where
  | nil : WfContext []
  | cons Γ x T : WfContext Γ → x ∉ Γ.fv → LcTy T → WfContext ((x, T) :: Γ)
...
/-
  Well-typed terms have locally closed terms and types.
-/
theorem typing_regularity {Γ t T} (h : Γ ⊢ t ∶ T) : LcTm t ∧ LcTy T := by
  constructor
  · apply typing_regularity_tm h
  · apply typing_regularity_ty h
